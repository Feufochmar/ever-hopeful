...
var gitMech = {
  ...
  setup: function (gitBin, repoDir, repoDocSubdir, refspec, callback) {
    ...
    childProcess.exec(this.gitBin + ' --version', function (err, stdout, stderr) {
      ...
      try {
        ...
        // Directory does not exist: create it (it does not need to be committed, as git does not track directory existance)
        try {
          fs.mkdirSync(repoDir + '/' + docSubdir)
        } catch (e) {
          callback('Cannot create document subdirectory: ' + repoDir + '/' + docSubdir)
          return
        }
      }
  ...
  show: function (path, version, callback) {
    gitSpawn(['show', version + ':' + path], function (err, data) {
      if (err) {
        callback(err)
      } else {
  ...
  log: function (path, version, howMany, callback) {
    ...
    gitSpawn(['log', '-' + howMany, '--reverse', '--no-notes', '--pretty=format:%h%n%H%n%an%n%ae%n%aD%n%ar%n%at%n%s', version, '--', path], function (err, data) {
  ...
  hashes: function (path, howMany, callback) {
    gitSpawn(['log', '-' + howMany, '--reverse', '--no-notes', '--pretty=format:%h', '--', path], function (err, data) {
  ...
  add: function (path, message, author, callback) {
    gitSpawn(['add', path], function (err) {
      if (err) {
        console.log(err)
        callback(err)
      } else {
  ...
  mv: function (path, newPath, message, author, callback) {
    gitSpawn(['mv', '-f', path, newPath], function (err) {
      if (err) {
        console.log(err)
        callback(err)
      } else {
        // Path is not given as there are two files to commit (removal of the old one, addition of the new one)
        this.commit(undefined, message, author, callback)
      }
    }.bind(this))
  },
  ...
  commit: function (path, message, author, callback) {
    var options
    if (path) {
      options = ['commit', '--author="' + author + '"', '-m', message, path]
    } else {
  ...
  grep: function (pattern, callback) {
    ...
    gitSpawn(args, function (err, data) {
      ...
      gitSpawn([ 'ls-files', docSubdir ], function (err, data) {
        if (data) {
