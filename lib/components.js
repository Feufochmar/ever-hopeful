...
Component.prototype.exists = function () {
  const docSubdir = this.getConfig().application.docSubdir
  this.file = docSubdir + '/' + this.getConfig().customizations[this.name]

  // The user can provide footer and sidebar without extension,
  // so we add a default '.md' in that case. This test is 'good enough' for Ever-Hopeful
...
module.exports = (function () {
  ...
  publicMethods.sidebarAsync = util.promisify(publicMethods.sidebar)
  publicMethods.footerAsync = util.promisify(publicMethods.footer)
