...
module.exports.initialize = function (config) {
  ...
  app.use(function (req, res, next) {
    res.locals = {
      ...
      hasGravatar: function () {
        return config.get('features').gravatar && req.user && req.user.email && req.user.email !== 'no.email.user'
      },
      hasFavicon: function () {
        return config.get('application').favicon && config.get('application').favicon.trim().length > 0
      },
      indexPage: function () {
        return config.get('pages').index
      },
      get isAjax () {
  ...
  app.use(session({
    name: 'ever.hopeful.sid',
    secret: config.get('application').secret,
    cookie: cookie,
    saveUninitialized: false,
    store: new MemoryStore({
      checkPeriod: 24 * 60 * 60 * 1000 // check every day
    }),
    resave: false
  }))
  app.use(flash())
  ...
  app.all('/assets/*', requireAuthentication)
  app.all('/uploadFile', requireAuthentication)

  if (!app.locals.config.get('authorization').anonRead) {
    app.all('/list', requireAuthentication)
    app.all('/wiki/*', requireAuthentication)
    app.all('/history/*', requireAuthentication)
    app.all('/version/*', requireAuthentication)
    app.all('/compare/*', requireAuthentication)
    app.all('/uploads', requireAuthentication)
    app.all('/uploads/*', requireAuthentication)
