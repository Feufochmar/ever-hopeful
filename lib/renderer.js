...
Marked.setOptions({
  ...
  renderer: mdRenderer,
  pedantic: false,
  // breaks: Set later, as it depends on configuration
  tables: true,
  smartLists: true,
  headerIds: true,
...
function evalTags (text) {
  ...
  for (var k in tagmap) {
    if (tagmap.hasOwnProperty(k)) {
      ...
      // Note: since this is done as markdown post-processing, some things may have previously been HTML-encoded
      pageName = he.decode(pageName)
...
function sanitize(text) {
  // Content should not have HTML in it
  const map = {
    '&': '&amp;',
    '<': '&lt;',
    '>': '&gt;',
...
var Renderer = {
  ...
  render: function (content) {
    ...
    var text = applyDirectives(sanitize(content))
