...
Page.prototype.renameTo = function (newName) {
  ...
  return new Promiserr(function (resolve, reject) {
    ...
    if (fs.existsSync(gitmech.absPath(pathNewFilename))) {
      reject()
      return
    }

    gitmech.mv(this.relativePathName,
               pathNewFilename,
...
Page.urlForShow = function (name, proxyPath) {
...
Page.prototype.urlForShow = function () {
  return Page.urlForShow(this.wikiname, this.getProxyPath())
}
...
Page.prototype.urlForRevert = function () {
  var wname = encodeURIComponent(this.wikiname)
  var proxy = this.getProxyPath() || ''
  return proxy + '/pages/' + wname + '/revert'
}

Page.prototype.urlForHistory = function () {
  var wname = encodeURIComponent(this.wikiname)
  var proxy = this.getProxyPath() || ''
  return proxy + '/wiki/' + wname + '/history'
}

Page.prototype.urlForCompare = function () {
  var wname = encodeURIComponent(this.wikiname)
  var proxy = this.getProxyPath() || ''
  return proxy + '/wiki/' + wname + '/compare'
}
